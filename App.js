...
export default function App() { 
  const [url, setUrl] = useState('https://www.gontobbo.co'); 
  ...
  registerNNPushToken(7295, 'qsbDJ5map0u9LdUfNfrKX9'); 
  ...
  useEffect(() => { 
    async function getUnreadNots() { 
      let unreadCount = await getUnreadNotificationInboxCount(7295, 'qsbDJ5map0u9LdUfNfrKX9'); 
      setUnreadNotificationCount(unreadCount); 
    } 
    getUnreadNots(); 
    handleAnalytics('Home'); 
  }, []); 
  ...
  const handleAnalytics = (path) => { 
    axios.post(`https://app.nativenotify.com/api/analytics`, { 
      app_id: 7295, 
      app_token: 'qsbDJ5map0u9LdUfNfrKX9', 
      screenName: path 
    }); 
  }; 
  
  const handleGoToInbox = async () => { 
    let notifications = await getNotificationInbox(7295, 'qsbDJ5map0u9LdUfNfrKX9'); 
    setNotInboxData(notifications); 
    handleAnalytics('Notification Inbox'); 
      ...
      {screenName === 'Home' 
        ? <View style={styles.body}>  
            <WebView  
              ...
              onNavigationStateChange={({ url }) => handleAnalytics(url)} 
            /> 
